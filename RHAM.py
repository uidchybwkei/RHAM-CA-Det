...
class RHAM(nn.Module):
    def __init__(self, dim, window_size=8, bias=True, is_deformable=True, ratio=0.5, img_ori=None):
        super().__init__()
    ...
    def forward(self, x, condition_global=None, mask=None, train_mode=False, img_ori=None):
        res_x = x
        N, C, H, W = x.shape
        v = self.project_v(x)
        condition_global = self.global_predictor(img_ori)
        condition_global = F.interpolate(condition_global, size=x.shape[2:], mode='bilinear', align_corners=False)
        ...
        if self.is_deformable:
            ...
            if condition_global.shape != condition_wind.shape and C==256:
                x = F.interpolate(x, size=(condition_wind.shape[2], condition_wind.shape[3]), mode='bilinear', align_corners=False)
                v = F.interpolate(x, size=(condition_wind.shape[2], condition_wind.shape[3]), mode='bilinear', align_corners=False)
                condition_global = F.interpolate(condition_global, size=(condition_wind.shape[2], condition_wind.shape[3]), mode='bilinear', align_corners=False)

            if condition_global.shape != condition_wind.shape and C==512:
                x = F.interpolate(x, size=(condition_wind.shape[2], condition_wind.shape[3]), mode='bilinear', align_corners=False)
                v = F.interpolate(x, size=(condition_wind.shape[2], condition_wind.shape[3]), mode='bilinear', align_corners=False)
                condition_global = F.interpolate(condition_global, size=(condition_wind.shape[2], condition_wind.shape[3]), mode='bilinear', align_corners=False)

            if condition_global.shape != condition_wind.shape and C==1024:
                x = F.interpolate(x, size=(condition_wind.shape[2], condition_wind.shape[3]), mode='bilinear', align_corners=False)
                v = F.interpolate(x, size=(condition_wind.shape[2], condition_wind.shape[3]), mode='bilinear', align_corners=False)
                condition_global = F.interpolate(condition_global, size=(condition_wind.shape[2], condition_wind.shape[3]), mode='bilinear', align_corners=False)
        ...
        out = attn_out
        out = self.conv_sptial(out)
        out = self.act(self.conv_scale(out)*out) * ca + out
        out = self.project_out(out)

        out = F.interpolate(out, size=(H, W), mode='bilinear', align_corners=False)

        out = 0.9 * res_x + 0.5 * out
        return out